...
<%
	...
	try {
		...
		documentList = sqlSession.selectList(
							"BBSDocumentMapper.getGalleryList", document);
	} catch (Exception e) {
		// 에러가 난 경우 처리할 내용이 있다면 구현한다.
		logger.error(e.getMessage());
	} finally {
			...
			<div class="col-xs-6 col-sm-4 col-md-4 col-lg-3">
				<div class="thumbnail">
					<a href="read.jsp?document_id=<%=item.getId()%>">
						<img src="download.jsp?file=<%=item.getFile()%>&img_size=320" style="width:100%"/></a>
					<div class="caption">
						...
						<p>
							조회수: <%=item.getHit()%><br />
							등록일: <%=item.getRegDate().substring(0,10)%>
							<%
			            		// 신규 일 때, 라벨출력
			            		if (is_new == true) {
...
%>
	        <div class="text-center">조회된 글이 없습니다.</div>
<%
    } // end if
%>
		</div>
